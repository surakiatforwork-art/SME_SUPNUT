...
const state = { rows: [], filters: { district: readPreference(localStorage, 'sme.selectedDistrict'), account: '', status: '', query: '' } };

app.innerHTML = `
  <main class="shell"><header><div><p class="eyebrow">SME FIELD SALES</p><h1>บันทึกงานหน้าร้าน</h1><p class="sub">ค้นหาร้าน แล้วเลือกสถานะการซื้อ</p></div><button id="refresh" class="button secondary" type="button">↻ โหลดข้อมูลใหม่</button></header>
...
async function load() { $('#notice').textContent = 'กำลังโหลดข้อมูล…'; try { const data = await request(config.apiUrl, '', 'list'); if (!Array.isArray(data.rows)) throw new Error('API ส่งข้อมูลรายการร้านไม่ถูกต้อง'); state.rows = data.rows; $('#notice').textContent = `อัปเดตล่าสุด ${new Date().toLocaleTimeString('th-TH', {hour:'2-digit', minute:'2-digit'})}`; } catch (error) { state.rows = []; $('#notice').textContent = `โหลดข้อมูลไม่สำเร็จ: ${error.message}`; } setupFilters(); }
...
$('#entryForm').addEventListener('submit', async event => { event.preventDefault(); const error = $('#formError'); try { const value = validateValue($('#value').value); $('#save').disabled = true; $('#save').textContent = 'กำลังบันทึก…'; const data = await request(config.apiUrl, '', 'save', { identity: activeRow.identity, value }); activeRow.value = data.value; dialog.close(); $('#notice').textContent = 'บันทึกเรียบร้อย'; render(); } catch (e) { error.textContent = e.message; error.hidden = false; } finally { $('#save').disabled = false; $('#save').textContent = 'บันทึก'; } });
load();
