...
async function load() { $('#notice').textContent = 'กำลังโหลดข้อมูล…'; try { const data = await request(config.apiUrl, '', 'list'); if (!Array.isArray(data.rows)) throw new Error('API ส่งข้อมูลรายการร้านไม่ถูกต้อง'); state.rows = data.rows.map(row => state.queue[row.identity] ? { ...row, value: state.queue[row.identity], pendingSync: true } : row); $('#notice').textContent = `อัปเดตล่าสุด ${new Date().toLocaleTimeString('th-TH', {hour:'2-digit', minute:'2-digit'})}`; } catch (error) { state.rows = []; $('#notice').textContent = `โหลดข้อมูลไม่สำเร็จ: ${error.message}`; } setupFilters(); }
